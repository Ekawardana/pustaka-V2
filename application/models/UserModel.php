...
class UserModel extends CI_Model
{
    ...
    function json()
    {
        $this->datatables->select('id, name, email, password, image, role, is_active, date_created');
        $this->datatables->from('user');
        $this->datatables->where('role', 'member');
        //add this line for join
        //$this->datatables->join('table2', 'user.field = table2.field');
        // $this->datatables->add_column('action', anchor(site_url('user/read/$1'), 'Read') . " | " . anchor(site_url('pengguna/update/$1'), 'Update') . " | " . anchor(site_url('pengguna/delete/$1'), 'Delete', 'onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), 'id_pengguna');
        $this->datatables->add_column(
            'action',
            '<div class="btn-group">' .
                form_open('master/Anggota/detail/$1') .
                form_button(['type' => 'submit', 'title' => 'Detail', 'class' => 'btn btn-primary', 'content' => '<i class="fas fa-info-circle"> </i>']) .
                form_close() . "&nbsp;" .
                form_open('master/Anggota/update/$1') .
                form_button(['type' => 'submit', 'title' => 'Edit', 'class' => 'btn btn-warning', 'content' => '<i class="fas fa-pencil-alt"> </i>']) .
                form_close() . "&nbsp;" .
                form_open('master/Anggota/delete/$1') .
